<template>
  <div>
    <!-- page-nav -->
    <page-nav :show-back="true" :title="`${cloudInfo?.alias || cloudInfo?.name} 授权详情`" />
    <div class="cloud-detail">
      <t-card class="cloud-card" :bordered="false">
        <h2 class="cloud-detail-title">
          <t-icon name="internet" size="24"></t-icon>
          <copy-label class="ml-[5px]" :value="cloudInfo?.name" />
        </h2>
        <t-row class="cloud-detail-row">
          <t-col :span="1">授权名称：</t-col>
          <t-col :span="3">
            <dns-alias
              :dns-id="cloudInfo?.id"
              :dns-alias="cloudInfo?.alias || cloudInfo?.name"
              :show-icon="false"
              :on-complete="reloadDnsInfo"
            />
          </t-col>
        ...
        <t-row>
          <t-col :span="6"><t-divider>授权配置</t-divider></t-col>
        </t-row>
        <t-row class="cloud-detail-row">
...
import { getDnsInfo } from '@/api/dns';
...
const configJson = computed(() => cloudInfo.value?.providerConfigJson);
// 详情
...
const reloadDnsInfo = async () => {
  try {
    // 加载详情
    cloudInfo.value = await getDnsInfo(cloudId.value);
  } catch (err) {
    console.log(err);
  }
};
